...
async function checkTables() {
  console.log('Checking access to specific tables...');
  ...
  // Also try to get a list of tables from the database metadata
  console.log('\nTrying to get table list from pg_tables...');
  try {
    ...
    if (error) {
      console.error('Error calling pg_tables:', error.message);
    } else {
      console.log('Tables from pg_tables:', data);
    }
  } catch (err) {
    console.error('Exception when calling pg_tables:', err.message);
  }
}
